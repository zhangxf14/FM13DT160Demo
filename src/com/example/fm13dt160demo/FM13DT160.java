...
//		}
	...
	public byte[] getRandom() throws IOException{
		byte[] result=transceive(CUSTOMER_GET_RANDOM);
		return result;
	}
	/**
	...
	public byte[] writeMemory(int firstAddress,int dataNum,byte[] data) throws IOException {
		...
		byte[] parameter=new byte[3+dataNum+1];
		parameter[0]= (byte) (firstAddress >>8 & 0xFF);
		parameter[1]= (byte) (firstAddress & 0xFF);
		parameter[2]= (byte) dataNum;
		System.arraycopy(data, 0, parameter, 3,data.length);
	 ...
	 * @throws IOException
	 */
	public byte[] start_stopRTC(int cmd_cfg,byte[] psw) throws IOException{
		byte[] parameter=new byte[5];
		parameter[0]=(byte) cmd_cfg;
		System.arraycopy(psw, 0, parameter, 1,4);
	...
	public byte[] startRTC() throws IOException {
		byte[] result=start_stopRTC(0x00, new byte[]{0,0,0,0});
		return result;
	}
	/**
	 * stop RTC
	 ...
	 * @throws IOException
	 */
	public byte[] stopRTC() throws IOException {
//		byte[] random=getRandom();
		byte[] result=start_stopRTC(0x80,  new byte[]{0,0,0,0});
		return result;
	}
	/**
	 * 
...
//			System.arraycopy(parameter, 0, nfcVCommand, 3, parameter.length);
		}else {
...
//			System.arraycopy(parameter, 0, nfcVCommand, 2, parameter.length);
		...
		return nfcv.transceive(nfcVCommand);
	}
}
